...
//modified JM : UPPERCASE for IN and OUT folder to avoir duplicate folders on FTP
...
$TransferDirection = "SPToFTP"  # Change as needed

# SharePoint Settings
$SharePointSite         = "https://nixonnow.sharepoint.com/sites/businesscentralchannel"
$SharePointLibrary      = "Shared Documents"
$SPSourceFolder         = "3PL_Jobs/EU/Outbox2"
$SPArchiveFolder        = "3PL_Jobs/EU/Outbox2/Archive"
$SPDestinationFolder    = "3PL_Jobs/EU/Inbox"
...
# FTP/SFTP Settings
$Protocol           = "SFTP"
$FtpHost            = "mystock.fms-ea.com"  # Removed ftp:// prefix
$FtpPort            = 21
$FtpUser            = "nixon"
$FtpPass            = "9tfyhNVB!"
$FtpSourceFolder    = "/OUT"
...
$NetworkArchiveFolder   = "\\NIXON02\ftproot\Tigers\live\FromFMS\archive"
$NetworkTargetFolder    = "\\NIXON02\ftproot\Tigers\live\ToFMS\ship"

# Local & Log
$TempFolder = "C:\Temp\SP_Transfer_EU"
$LogFile = "C:\Logs\SP_Transfer_EU.log"
...
function SPToFTPGeneric {
    ...
    $HostKeyParam = ""
    if ($isSFTP) {
        $PinnedHostKey = 'ecdsa-sha2-nistp256 256 qLI4Er+3h7wEuAuMSWffpVJnckWm9egyz7ciWi4+GqI='
    ...
    foreach ($file in $files) {
        ...
        # Create the WinSCP script with proper formatting
        $scriptContent = @"
option batch abort
option confirm off
...
"@
        ...
        # Execute WinSCP
        $process = Start-Process -FilePath $WinSCP -ArgumentList "/ini=nul /script=`"$scriptPath`" /log=`"$TempFolder\winscp_upload.log`"" -Wait -PassThru -NoNewWindow
        ...
        if ($ExitCode -eq 0) {
            ...
            if (Test-Path "$TempFolder\winscp_upload.log") {
                Get-Content "$TempFolder\winscp_upload.log" -Tail 20 | ForEach-Object { Write-Log "WinSCP Log: $_" }
            }
        }
